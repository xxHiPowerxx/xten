 ...
 * @return array
 */
function xten_body_classes( $classes ) {
...
add_filter( 'body_class', 'xten_body_classes' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function xten_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'xten_pingback_header' );
 ...
 * @return array
 */
function xten_filter_script_loader_tag( $tag, $handle ) {
...
add_filter( 'script_loader_tag', 'xten_filter_script_loader_tag', 10, 2 );
 ...
 */
function xten_get_preload_stylesheet_uri( $wp_styles, $handle ) {
...
function xten_add_body_style() {
	...
	// Preload content.css.
	$preloads['xten-content-css'] = xten_get_preload_stylesheet_uri( $wp_styles, 'xten-content-css' );

	// Preload comments.css.
	if ( ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() ) ) {
		$preloads['xten-comments-css'] = xten_get_preload_stylesheet_uri( $wp_styles, 'xten-comments-css' );
	}

	// Preload front-page.css.
	global $template;
	if ( 'front-page.php' === basename( $template ) ) {
		$preloads['xten-front-page-css'] = xten_get_preload_stylesheet_uri( $wp_styles, 'xten-front-page-css' );
	}

	// Preload front-page.css.
	global $template;
	if ( '404.php' === basename( $template ) ) {
			$preloads['xten-404-css'] = xten_get_preload_stylesheet_uri( $wp_styles, 'xten-404-css' );
	}
 ...
 */
function xten_add_primary_menu_dropdown_symbol( $item_output, $item, $depth, $args ) {
	...
	if ( ! empty( $item->classes ) && in_array( 'menu-item-has-children', $item->classes ) && $depth == 0  ) {
		return $item_output . '<span class="dropdown fa fa-chevron-down"></span>';
	}

	return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'xten_add_primary_menu_dropdown_symbol', 10, 4 );
 ...
 */
function xten_add_nav_menu_aria_current( $atts, $item ) {
	/*
...
add_filter( 'nav_menu_link_attributes', 'xten_add_nav_menu_aria_current', 10, 2 );
add_filter( 'page_menu_link_attributes', 'xten_add_nav_menu_aria_current', 10, 2 );
